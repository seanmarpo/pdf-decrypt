 ...
 * AES-128 tests: decrypt static fixture (fixtures/aes128-encrypted.pdf, password: "password")
...
async function runTests() {
  ...
  try {
    ...
    assert(
      typeof isEncrypted === "function",
      "isEncrypted should be a function",
    );
  ...
  try {
    ...
    const encrypted = await encryptPDF(new Uint8Array(pdfBytes), "rc4pass", {
      algorithm: "RC4",
    });
    console.log(
      "  Original:",
  ...
  try {
    ...
    try {
      ...
      assert(
        ...
        "Should mention incorrect password",
      );
      console.log("  PASSED\n");
      passed++;
    }
  ...
  try {
    ...
    try {
      ...
      console.log("  Error (expected):", e.message);
      assert(
        e.message.includes("not encrypted"),
        "Should mention not encrypted",
      );
      console.log("  PASSED\n");
      passed++;
    }
  ...
  try {
    ...
    // RC4 re-encrypt
    const enc2 = await encryptPDF(new Uint8Array(dec1), "pass2", {
      algorithm: "RC4",
    });
    ...
    assert(
      doc.getPageCount() === 1,
      "Should have 1 page after double roundtrip",
    );
    console.log("  Pages:", doc.getPageCount());
    console.log("  PASSED\n");
    passed++;
  ...
  // Uses a static fixture because encryptPDF randomly produces ciphertext bytes
  // that confuse pdf-lib's parser during ignoreEncryption load, causing truncated
  // strings that silently fail AES-CBC decryption.
  try {
    ...
    const encrypted = fs.readFileSync(
      path.join(__dirname, "fixtures", "aes256-metadata-encrypted.pdf"),
    );
  ...
  // ========== AES-128 (V=4, R=4) Tests — Static Fixture ==========

  // Test 13: isEncrypted on AES-128 PDF
  try {
    console.log("Test 13: isEncrypted on AES-128 PDF");
    const pdfBytes = fs.readFileSync(
      path.join(__dirname, "fixtures", "aes128-encrypted.pdf"),
    );
    const info = await isEncrypted(new Uint8Array(pdfBytes));
    console.log("  Result:", JSON.stringify(info));
    assert(info.encrypted === true, "Should be encrypted");
    assert(info.algorithm === "AES-128", "Algorithm should be AES-128");
    assert(info.version === 4, "Version should be 4");
    assert(info.revision === 4, "Revision should be 4");
    assert(info.keyLength === 128, "Key length should be 128 bits");
    console.log("  PASSED\n");
    passed++;
  } catch (e) {
  ...
  // Test 14: AES-128 decrypt with correct password → valid PDF
  try {
    console.log("Test 14: AES-128 decrypt with correct password");
    const pdfBytes = fs.readFileSync(
      path.join(__dirname, "fixtures", "aes128-encrypted.pdf"),
    );
    const decrypted = await decryptPDF(new Uint8Array(pdfBytes), "password");
    console.log(
      "  Encrypted:",
      pdfBytes.length,
      "-> Decrypted:",
    ...
    const doc = await PDFDocument.load(decrypted);
    assert(doc.getPageCount() >= 1, "Should have at least 1 page");
    console.log("  Pages:", doc.getPageCount());
    console.log("  PASSED\n");
    passed++;
  } catch (e) {
  ...
  // Test 15: AES-128 wrong password → throws "Incorrect password"
  try {
    console.log("Test 15: AES-128 wrong password — should throw");
    const pdfBytes = fs.readFileSync(
      path.join(__dirname, "fixtures", "aes128-encrypted.pdf"),
    );

    try {
      await decryptPDF(new Uint8Array(pdfBytes), "wrongpassword");
      console.log("  FAILED: Should have thrown\n");
      failed++;
    } catch (e) {
      console.log("  Error (expected):", e.message);
  ...
  // Test 16: AES-128 decrypt preserves metadata strings
  try {
    console.log("Test 16: AES-128 metadata survives decryption");
    const pdfBytes = fs.readFileSync(
      path.join(__dirname, "fixtures", "aes128-encrypted.pdf"),
    );
    const decrypted = await decryptPDF(new Uint8Array(pdfBytes), "password");
    ...
    // Just verify metadata can be read without error (not null/undefined crash)
    assert(
      title === undefined || typeof title === "string",
      "Title should be a string or undefined",
    );
    console.log("  PASSED\n");
    passed++;
  } catch (e) {
